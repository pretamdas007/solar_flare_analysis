    ...
    def predict_flare_detection(self, data: np.ndarray) -> Dict:
        ...
        try:
            ...
            if 'cnn_detector' in self.models:
                cnn_pred = self.models['cnn_detector'].predict(data, verbose=0)
...
class DataProcessor:
        ...
        try:
            ...
            for i, peak_idx in enumerate(peaks):
                ...
                flare_info = {
                    ...
                    'class': flare_class,
                    'start_time': times[start_idx].isoformat(),
                    'end_time': times[end_idx].isoformat(),
                    'duration_minutes': float((times[end_idx] - times[start_idx]).total_seconds() / 60),
                    'index': int(peak_idx)
                }
    ...
    def prepare_ml_input(self, window_size: int = 100) -> np.ndarray:
        ...
        features = np.column_stack([
            xrs_a_log[-window_size:],
            xrs_b_log[-window_size:],
            data['ratio'].values[-window_size:]
        ])
