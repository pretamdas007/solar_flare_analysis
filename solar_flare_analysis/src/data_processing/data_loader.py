...
class GOESDataLoader:
    ...
    def __init__(self, cache_dir='data_cache'):
        ...
        self.base_urls = {
            'GOES-16': 'https://data.ngdc.noaa.gov/platforms/solar-space-observing-satellites/goes/goes16/l2/data/xrsf-l2-avg1m_science/'
        }
    ...
    def load_from_files(self, file_paths, start_date=None, end_date=None):
        ...
        # Ensure all DataFrames have consistent index types before concatenation
        processed_data = []
        for data in all_data:
            if data is not None:
                ...
                if not isinstance(data.index, pd.DatetimeIndex):
                    try:
                        # Check if index contains datetime-like values
                        if hasattr(data.index, 'to_pydatetime') or any(isinstance(idx, (pd.Timestamp, np.datetime64)) for idx in data.index[:5]):
                            data.index = pd.to_datetime(data.index)
                        else:
                            # If index is numeric, skip this data or reset index
                            print(f"Warning: Skipping data with non-datetime index type: {type(data.index)}")
                            continue
                    except Exception as e:
                        print(f"Warning: Could not convert index to datetime, skipping data: {e}")
                        continue
        ...
        # Combine all data
        combined_data = pd.concat(processed_data, ignore_index=False).sort_index()
    ...
    def _load_netcdf_file(self, file_path):
        ...
        try:
            with xr.open_dataset(file_path) as ds:
                ...
                # Extract XRS data with improved mapping logic
                for var in xrs_vars:
                    var_lower = var.lower()
                    # Handle specific GOES variable naming patterns
                    if any(pattern in var_lower for pattern in ['xrsa', 'xrs_a', 'xrs-a']) and 'long' not in var_lower:
                        data_dict['xrs_a'] = ds[var].values
                    elif any(pattern in var_lower for pattern in ['xrsb', 'xrs_b', 'xrs-b']) and 'long' not in var_lower:
                        data_dict['xrs_b'] = ds[var].values
                    # Fallback for generic patterns (maintaining backward compatibility)
                    elif 'xrs' in var_lower and 'a' in var_lower and 'long' not in var_lower and 'xrs_a' not in data_dict:
                        data_dict['xrs_a'] = ds[var].values
                    elif 'xrs' in var_lower and 'b' in var_lower and 'long' not in var_lower and 'xrs_b' not in data_dict:
                        data_dict['xrs_b'] = ds[var].values
    ...
    def _apply_quality_filter(self, data):
        ...
        for col in xrs_cols:
            ...
            z_scores = np.abs(zscore(filtered_data[col].dropna()))
    ...
    def _remove_background_trend(self, data, window_size='1H', quantile=0.1):
        ...
        if isinstance(window_size, str):
            # Simple conversion for common time units
            if 'H' in window_size.upper():
                hours = int(window_size.upper().replace('H', ''))
                # Assuming 1-minute sampling, 1 hour = 60 samples
                window_samples = hours * 60
            elif 'min' in window_size.lower():
                minutes = int(window_size.lower().replace('min', ''))
                window_samples = minutes
            else:
                window_samples = min(120, len(data) // 10)  # Default fallback
        else:
            window_samples = window_size
        
        # Ensure odd window size for median filter
        window_samples = max(3, min(window_samples, len(data) // 2))
        ...
        for col in xrs_cols:
            ...
            if window_samples > 3:
                background = signal.medfilt(processed_data[col].values, kernel_size=window_samples)
                processed_data[f'{col}_detrended'] = processed_data[col] - background
                processed_data[f'{col}_background'] = background
                # Also create a column without background for compatibility
                processed_data[f'{col}_no_background'] = processed_data[col] - background
    ...
    def _generate_flare_profile(self, flare_type, max_length=1000):
        ...
        for i, time in enumerate(t):
            if time <= peak_time:
                ...
                profile_a[i] = peak_flux_a * (1 - np.exp(-time / (rise_time / 3)))
                profile_b[i] = peak_flux_b * (1 - np.exp(-time / (rise_time / 3)))
            else:
                # Decay phase - exponential decay
                decay_factor = np.exp(-(time - peak_time) / (decay_time / 3))
                profile_a[i] = peak_flux_a * decay_factor
                profile_b[i] = peak_flux_b * decay_factor
        return profile_a, profile_b


# Wrapper functions for backward compatibility with main.py
def load_goes_data(file_paths, start_date=None, end_date=None):
    """
    Load GOES data using the GOESDataLoader class
    ...
    -------
    pd.DataFrame
        Loaded and basic preprocessed data
    """
    loader = GOESDataLoader()
    if isinstance(file_paths, str):
        file_paths = [file_paths]
    return loader.load_from_files(file_paths, start_date, end_date)


def preprocess_xrs_data(data, resample_freq='1min', apply_quality_filter=True, 
                       normalize=True, remove_background=True):
    """
    Preprocess XRS data using the GOESDataLoader class
...
def remove_background(data, window_size='1H', quantile=0.1):
    """
    ...
    data : pd.DataFrame
        Input data
    window_size : str or int, optional
        Window size for background estimation (default: '1H')
    quantile : float, optional
        Quantile for background estimation (default: 0.1)
